...
![グラフマップによる全体把握](docs/widemap1.png)
![統計情報ビュー](docs/widemap2.png)
![通信ログのドリルダウン](docs/widemap3.png)
![端末一覧のドリルダウン](docs/widemap4.png)
...
> **注意:** 認証情報は初回起動時に1度だけ生成され、（ハッシュ化して）`.widemap.json` に保存されます。パスワードを紛失した場合は `.widemap.json` の `auth` セクションを削除して再起動すれば、新しい初期パスワードが表示されます。
...
| **API トークン** | スクリプト・自動化（`X-Admin-Token` ヘッダー） | `.widemap.json`（`adminToken`）。再生成は 設定 → 一般 |
...
デフォルトは HTTP です。HTTPS を有効にするには `.widemap.json` に以下を追加して再起動します：
...
複数端末からパスワードでログインする場合は、LAN 内の盗聴から認証情報と画面データを守るため HTTPS の有効化を推奨します。
