...
### 2026-06-09 セッション（継続）で完了した実装

| 機能 | 内容 |
|------|------|
| soft delete による merge 永続化 | `approveMerge` の DELETE → `archivedAt`/`mergedInto` による論理削除に変更。`seedFromConnectionHistory` が接続履歴から旧 IP を upsert しても archivedAt が保持され、merge 結果が再起動後も維持される |
| archivedAt/mergedInto スキーマ追加 | `devices` テーブルに `archivedAt INTEGER` と `mergedInto TEXT` を追加（ALTER TABLE 移行込み）。P1-8 task 4（手動 archive）の土台が整備済み |
| getAll / observeDevice での archived 除外 | `stmtSelectAll` に `WHERE archivedAt IS NULL`。`observeDevice` は archived IP で early return。`stmtSelectMac` / `stmtByMdns` / `stmtByDns` も archived 除外 |
| stable-MAC 重複 21 グループの再マイグレーション | soft delete 対応後に EC2 上で再実行。113 → 92 台（archived: 21）。2回の `systemctl restart` で 92/21 を維持確認 |
...
| P1-6 端末一覧タブ | `GET /api/devices` API（NDP IPv6付き）と「🖥 端末一覧」ビューを追加。検索・ソート・更新ボタン対応 |
...
| 🔶 P1-8 | 端末 inventory の active / stale / archive 整理（**一部完了**） | 実LAN端末は約69台だが端末一覧は106件あった（stable-MAC 21グループ統合後は **92台** active / 21 archived）。今後も DHCP再割当・private MAC・旧IPの残骸が積もるため、active/stale/archived の状態管理が必要 | **完了済み**: `archivedAt`/`mergedInto` カラム追加・approveMerge soft delete 化・21グループ統合（2026-06-09）。**未実装**: `GET /api/devices` の `status` フィールド・端末一覧の active/stale/archived フィルタ UI（タスク1〜3）・端末詳細の archive/unarchive ボタン（タスク4 UI）|
...
| 1d | `devices_new` でテーブル作り直し（段階移行④）**※実施トリガー待ち** | `deviceId TEXT PRIMARY KEY`、`ip TEXT`（PRIMARY KEY 廃止）で再作成。現状は `deviceId UNIQUE INDEX` で一意性が担保され、アプリ層も deviceId を使っているため直ちに実施しない。**実施トリガー**（いずれか）: (1) `ON CONFLICT(deviceId)` 主軸の upsert が必要になった / (2) ip nullable・複数IP保持など ip PRIMARY KEY が実装上の制約になった / (3) ORM・migration tool 導入時に物理スキーマを整理したくなった / (4) バックアップ/復元を含む migration 手順を十分に検証できた。**トリガーが来なければ ip PRIMARY KEY のまま維持してよい** | ip PRIMARY KEY が消え deviceId PRIMARY KEY になる。既存データが全件移行される |
...
実LAN端末は約69台。端末一覧は stable-MAC 重複 21グループの統合（2026-06-09）で **92台**（active）/ 21 archived に整理済み。`archivedAt`/`mergedInto` カラムも追加済みで soft delete 基盤は完成している。今後も DHCP 再割当・private MAC・旧 IP の残骸が積もるため、active/stale/archived の状態管理と表示フィルタを整備する。まず削除ではなく、表示と状態管理で「今いる端末」と「過去に見た端末」を分ける。
...
| 2 | 端末一覧フィルタ | active/recent/stale/archived の表示切替を追加。既定は active + recent | active 台数が見える（現在 92台）。実LAN台数に近い数字で把握できる |
| 3 | stale 端末の扱い | stale は削除せず、merge候補・過去履歴・メモ参照用に残す | stale を非表示にしても履歴・notes・merge candidate は壊れない |
| 4 | 手動 archive / unarchive | 端末詳細から archive / unarchive できるようにする | archive 端末は通常一覧から外れ、必要時だけ表示できる。`archivedAt`/`mergedInto` カラムは追加済み（2026-06-09）。残作業: (a) 端末詳細 UI に archive / unarchive ボタン追加。(b) archived IP の観測を `mergedInto` 側へ redirect する処理（現状は drop）— unarchive 実装と同時に入れると整合しやすい |
